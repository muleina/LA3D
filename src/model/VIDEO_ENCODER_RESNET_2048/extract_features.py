...
def load_frame(frame_file, isfrom_file=True):
    """
    ...
    if isfrom_file:
        ...
        # data = frame_file
        # print(data.shape)
        # data = cv2.resize(data, (340, 256)) # to ensure resize operation much with PIL
        data = Image.fromarray(frame_file)

    data = data.resize((340, 256), Image.ANTIALIAS) # WxH
    # print(data.size) # WxH
...
def load_rgb_batch(frames_dir, rgb_files, frame_indices, isfrom_file=True):
    batch_data = np.zeros(frame_indices.shape + (256,340,3))
    for i in range(frame_indices.shape[0]):
        for j in range(frame_indices.shape[1]):
            batch_data[i,j,:,:,:] = load_frame(os.path.join(frames_dir, rgb_files[frame_indices[i][j]]) if isfrom_file else rgb_files[frame_indices[i][j]], isfrom_file=isfrom_file)
        # break
    return batch_data


def oversample_data(data, T=10):
...
def run(i3d, frequency, frames_dir, rgb_files, batch_size, sample_mode, oversample_T=10):
    ...
    print(f"adjusted batchsize: {batch_size}")
    batch_num = int(np.ceil(chunk_num / batch_size))    # Chunks to batches
    frame_indices = np.array_split(frame_indices, batch_num, axis=0)

    if sample_mode == 'oversample':
        full_features = [[] for i in range(oversample_T)]
    else:
        full_features = [[]]


    for batch_id in tqdm(range(batch_num)): 
        batch_data = load_rgb_batch(frames_dir, rgb_files, frame_indices[batch_id], isfrom_file=isfrom_file)
        if(sample_mode == 'oversample'):
            batch_data_ten_crop = oversample_data(batch_data, T=oversample_T)
            for i in range(oversample_T):
