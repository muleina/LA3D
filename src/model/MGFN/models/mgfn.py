...
def MSNSD(features,scores,bs,batch_size,drop_out,ncrops,k, infer=False):
    ...
    if nfea_magnitudes.shape[0] == 1 or infer:  # this is for inference
...
class mgfn(nn.Module):
    ...
    def forward(self, video):
        # k = 3 # only supports bs >= 3
        k = min(3, video.shape[0]) # mulugeta to support inference bs >= 1 
        bs, ncrops, t, c = video.size()
        x = video.view(bs * ncrops, t, c).permute(0, 2, 1)
        # x_f = x[:,:2048,:]
        # x_m = x[:,2048:,:]
        x_f = x[:,:self.to_tokens.in_channels,:] # mulugeta, to support xd model
        x_m = x[:,self.to_tokens.in_channels:,:]
        ...
        score_abnormal, score_normal, abn_feamagnitude, nor_feamagnitude, scores  = MSNSD(x,scores,bs,self.batch_size,self.drop_out,ncrops,k, infer=self.infer)
